...
#'
#' @param object An object of class  \code{localtest}
...
#' # --------------------------------------
#' #   Backgound pattern X: 20
#' # Alternative pattern Z: 17
#' #
...
summary.localtest <- function(object, ...){
  if(!any(class(object) == "localtest")) stop("class(object) must be localtest")
  ...
  cat(paste("Backgound pattern X:",nrow(object$X$df), " \n"))
