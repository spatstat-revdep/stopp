...
#'
stlgcppm <- function(X, formula, verbose = TRUE, cov = "separable",
                   first = "local", second = "local", mult = 4,
                   hs = "global", npx0 = 10, npt0 = 10, itnmax = 100){
  time1 <- Sys.time()
  nX <- nrow(X$df)
  ...
  g0 <- if(first == "global"){
    stpp::LISTAhat(xyt = as.stpp(X), lambda = exp(pred_global), s.region = s.region, t.region = t.region,
             dist = u, times = v, ks = "epanech", hs = us0, kt = "box",
             ht = vt0)
  } else {
    stpp::LISTAhat(xyt = as.stpp(X), lambda = exp(pred_local), s.region = s.region, t.region = t.region,
             dist = u, times = v, ks = "epanech", hs = us0, kt = "box",
             ht = vt0)
  }
  ...
  if(cov == "separable"){
    ...
    min_vals <- c(0.01, 0.001, 20)

    max_vals <-  c(50, 10, 1500)
  ...
  if(cov == "gneiting") {
    ...
    max_vals <- c(nX, max_dist * 250, max_dist_t * 250, 2, 2, 2)
  ...
  if(cov == "iaco-cesare") {
    ...
    max_vals <- c(nX, max_dist * 250, max_dist_t * 250, 2, 2, 15)
